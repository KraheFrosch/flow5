...
// GPU Matrix Export - Commented out as not currently needed
/*
double P3UniAnalysis::getInfluenceCoefficient(int iPanelObs, int jPanelSrc) const
{
    // Replicates logic from makeMatrixBlock and makeWakeMatrixBlock
    // to compute a single matrix element A[i,j] on CPU
    
    double val = 0.0;
    Panel3 const &p3i = m_Panel3.at(iPanelObs);
    Panel3 const &p3k = m_Panel3.at(jPanelSrc);
    ...
    if(m_pPolar3d->bNeumann() || p3i.isMidPanel())
    {
        Vector3d Vb[3];
        p3k.doubletBasisVelocity(p3i.CoG(), Vb);
        Vector3d vel = Vb[0]+Vb[1]+Vb[2];
        val = vel.dot(p3i.normal());
        ...
        if(m_pPolar3d->bGroundEffect() || m_pPolar3d->bFreeSurfaceEffect())
        {
            ...
            Vector3d  CG(p3i.CoG().x, p3i.CoG().y, -p3i.CoG().z-2.0*m_pPolar3d->groundHeight());
            p3k.doubletBasisVelocity(CG, Vb);
            Vector3d velG = Vb[0]+Vb[1]+Vb[2];
            velG.z = -velG.z;
            val += velG.dot(p3i.normal()) * coef;
        }
    }
    else if(m_pPolar3d->bDirichlet())
    {
        double phib[]{0,0,0};
        p3k.doubletBasisPotential(p3i.CoG(), iPanelObs==jPanelSrc, phib, true);
        double phiNasa = phib[0]+phib[1]+phib[2];
        val = phiNasa;
    ...
    // 2. Wake Contribution
    // Need to find if jPanelSrc triggers a wake that affects iPanelObs
    // The wake logic in makeWakeMatrixBlock iterates over ALL trailing panels (p3k)
    // and adds to columns j related to the wake.
    // Here we have a specific jPanelSrc. We need to know if this panel is part of a wake relationship.
    
    // Inverse logic: Does jPanelSrc contribute to the wake influence?
    // In makeWakeMatrixBlock, wake calc is done for each trailing panel column.
    // If p3k is MidPanel: adds to column k3 (jPanelSrc)
    // If p3k is BotPanel: adds to column k3 (jPanelSrc) AND k3t (Opposite)
    
    // So if jPanelSrc is a MidPanel or BotPanel that is trailing, it contributes directly.
    // If jPanelSrc is a TopPanel that is trailing, it receives contribution from the BotPanel's wake calc.
    
    // It's easier to iterate the trailing panels to see if they involve jPanelSrc
    // But direct check is O(1) vs O(N_trailing).
    
    if (p3k.isTrailing()) { 
        if (p3k.isMidPanel() || p3k.isBotPanel()) {
             // Calculate wake influence generated by THIS panel's wake strip
             // MatWakeContrib calculation...
             // Add to val
             ...
             do {
                ...
                if(m_WakePanel3.at(jWake).iPD()<0) break;
                jWake++;
             } while(jWake<nWakePanels());
             
             if (p3k.isMidPanel()) {
                 val += MatWakeContrib;
             } else { // BotPanel
                 val += MatWakeContrib * (-1.0);
             }
        }
        else if (p3k.isTopPanel()) {
             // For a Top Trailing Panel, the contribution comes from the corresponding Bottom Panel
             int k3b = p3k.oppositeIndex();
             if (k3b >= 0) {
                 Panel3 const &p3b = m_Panel3.at(k3b);
                 if (p3b.isTrailing() && p3b.isBotPanel()) {
                     // Calculate wake influence generated by the BOTTOM panel
                     double MatWakeContrib = 0.0;
                     int jWake = p3b.iWake();
                     ...
                     val += MatWakeContrib; // Add to Top Panel column
                 }
             }
        }
    }
